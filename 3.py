...
class SavingAccount(Account):
    def deposit(self, amount):
        if amount > 0:
            self.balance += amount
            print(f'Deposit {amount} to saving {self.account_number}.New balance: {self.balance}')
        else:
    ...
    def withdraw(self, amount):
        if 0 < amount <= self.balance:
            self.balance -= amount
            print(f'Withdraw {amount} to saving {self.account_number}.New balance: {self.balance}')
        else:
...
class ChekingAccount(Account):
    ...
    def withdraw(self, amount):
        if amount > self.withdraw_limit:
            print(f'Ошибка! Сумма превышает допустимый лимит')
        elif self.balance-amount-self.tr_fee < self.min_balance:
            print(f'Ошибка! Сумма превышает мин баланс')
    ...
    def checking_accounts(self, customer):
        ...
        if total >= 2000000:
            ...
            return VIPCustomer(customer.name, customer.accounts)
        return customer
...
class Customer():
    ...
    def transfer(self, from_account, to_account, amount):
        if amount <= from_account.balance:
            ...
            print(f'Transfer {amount} from {from_account.account_number} to {to_account.account_number}')
        else:
            print('ERROR')

class VIPCustomer(Customer, ChekingAccount):
    def __init__(self, name, accounts=None):
        Customer.__init__(self, name)
        ChekingAccount.__init__(self)
    ...
    def apply_interset(self):
        for account in self.accounts:
            if isinstance(account, SavingAccount):
                ...
                print(f'{interset}. New balance {account.balance}')

saving = SavingAccount(1000000, 'C1234')
cheking = ChekingAccount(1200000, 'SA456', min_balance=500, withdraw_limit=10000, tr_fee=50)
